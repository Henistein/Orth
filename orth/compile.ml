...
let type_unop_ident st cmd = 
    match st, cmd with
    | Tident :: rest, Ops Call  -> rest
    | Tident :: rest, Ops Fetch -> rest
    | t1 :: rest, _             -> raise (TypeError (unop_exc cmd t1))
    | _, _                      -> raise (TypeError (need_two_elems cmd))
...
let rec comprec = function
    ...
    | Ifelse (b1, b2) ->
        ...
        let body1 = (List.fold_left (++) nop (List.map comprec (List.rev b1))) in 
        
        if stack_before_b1 <> !stack_types then 
            raise (TypeError "A pilha deve ser a mesma, antes e depois do corpo do if")
        else (
            let stack_before_b2 = !stack_types in
            (* compila o corpo do else *)
            let body2 = (List.fold_left (++) nop (List.map comprec (List.rev b2))) in
            if stack_before_b2 <> !stack_types then 
                raise (TypeError "A pilha deve ser a mesma, antes e depois do corpo do else")
            else (
    ...
    | While (c, b) ->
        ...
        (* Compila a condicao *)
        let cond = (List.fold_left (++) nop (List.map comprec (List.rev c))) in
        (* Verificar se o ultimo elemento da pilha é um bool *)
        stack_types := (check_while_cond !stack_types);
        let w_num = !while_index in
        let stack_before_b1 = !stack_types in
        (* compila o corpo do while *)
        let body1 = (List.fold_left (++) nop (List.map comprec (List.rev b))) in
    ...
    | Proc (b) ->
        ...
        List.rev b 
        |> List.map (fun x -> Printf.printf "\t"; comprec x) 
        |> List.fold_left (++) nop 
        |> Hashtbl.add procs_tbl s;
...
let compile_program p ofile =
    ...
    (*  "flush" do buffer para garantir que tudo fica escrito antes do fecho deste  *)
    fprintf fmt "@?";
    close_out f
